    ...
    ui(new Ui::TweClock)
{
    ...
    void (QWidget::*qw_update)() = &QWidget::update;
    connect(timer,&QTimer::timeout,this,qw_update);//定时调用绘制事件函数
}
...
//初始化参数函数,使用系统默认时间
void TweClock::Init_Parameter(QTime time)
{
//    Center_pos = QPoint(this->width()/2,this->height()/2);//以窗体中心为圆心
//    R_Edge = this->height()/2;//以窗体高度的一半为半径
